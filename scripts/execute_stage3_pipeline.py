...
def main():
    ...
    if not os.path.exists(mapping_csv):
        raise FileNotFoundError(
            f"Authoritative validation mapping CSV missing at '{mapping_csv}'"
        )
    ...
    print(
        f"[OK] Authoritative 320-sample validation mapping locked (SHA256: {mapping_sha256})"
    )

    # Reconstruct Training Split (2880 files)
    config = Config(MODEL_VERSION="AIR-Net-v1.2")
    lr_files = sorted(
        [f for f in os.listdir(config.train_lr_dir) if f.endswith(".npy")]
    )
    gt_files = sorted(
        [f for f in os.listdir(config.train_gt_dir) if f.endswith(".npy")]
    )
    common_files = sorted(list(set(lr_files).intersection(set(gt_files))))

    assert (
        len(common_files) == 3200
    ), f"Expected 3200 paired files, got {len(common_files)}"
    val_set_filenames = set(val_filenames)
    train_filenames = sorted([f for f in common_files if f not in val_set_filenames])
    assert (
        len(train_filenames) == 2880
    ), f"Expected 2880 train files, got {len(train_filenames)}"

    # Data Loaders
    train_ds = KLAPairedNpyDataset(
        train_filenames, config.train_lr_dir, config.train_gt_dir
    )
    val_ds = KLAPairedNpyDataset(
        val_filenames, config.train_lr_dir, config.train_gt_dir
    )
    ...
    if os.path.exists(state_ckpt_path):
        ...
        print(
            f"Resumed at Epoch {start_epoch} (Previous Best PSNR: {best_val_psnr:.4f} dB)"
        )
    else:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        with torch.no_grad(), torch.inference_mode():
            for lr_b, gt_b, _ in val_loader:
                ...
                pred = torch.clamp(
                    out["restored"] if isinstance(out, dict) else out, 0.0, 1.0
                )
        ...
        print(
            f"Epoch [{epoch:02d}/{total_epochs:02d}] ({epoch_time:.1f}s) | LR: {current_lr:.6f}"
        )
        print(
            f"  Train Loss: {avg_train_loss:.4f} (L1: {avg_l1:.4f}, SSIM: {avg_ssim_loss:.4f}, Edge: {avg_edge_loss:.4f}, HF: {avg_hf_loss:.4f})"
        )
        print(
            f"  Val Loss:   {avg_val_loss:.4f} | PSNR: {avg_val_psnr:.4f} dB | SSIM: {avg_val_ssim:.4f} | LPIPS: {avg_val_lpips:.4f}"
        )
        print(
            f"  Analytical: HF Ret: {avg_hf_retention:.6f} | Grad: {avg_grad_energy:.8f} | Lap: {avg_lap_energy:.8f}"
        )
        ...
        with open(history_csv_path, "a", newline="") as f:
            ...
            writer.writerow(
                [
                    ...
                    round(avg_l1, 6),
                    round(avg_ssim_loss, 6),
                    round(avg_edge_loss, 6),
                    round(avg_hf_loss, 6),
                    round(avg_val_loss, 6),
                    round(avg_val_psnr, 4),
                    round(avg_val_ssim, 4),
                    round(avg_val_lpips, 4),
                    round(avg_hf_retention, 6),
                    round(avg_grad_energy, 8),
                    round(avg_lap_energy, 8),
                    f"{current_lr:.6e}",
                ]
            )
        ...
        if epoch in [1, 5, 10, 15, 20]:
            ...
            # Samples 000001, 000021, 000034, 000064, 000095
            ...
            for idx in demo_indices:
                ...
                axes[0, 0].set_title(
                    f"Input 128x128 ({bname})", fontsize=11, fontweight="bold"
                )
                axes[0, 0].axis("off")
                ...
                axes[0, 2].set_title(
                    f"AIR-Net v1.2 (Epoch {epoch})", fontsize=11, fontweight="bold"
                )
                axes[0, 2].axis("off")

                axes[1, 0].imshow(gt_np, cmap="gray", vmin=0, vmax=1)
                axes[1, 0].set_title(
                    "Ground Truth 256x256", fontsize=11, fontweight="bold"
                )
                axes[1, 0].axis("off")

                axes[1, 1].imshow(err_np, cmap="inferno")
                axes[1, 1].set_title(
                    "Absolute Error Map", fontsize=11, fontweight="bold"
                )
                axes[1, 1].axis("off")

                axes[1, 2].imshow(edge_np, cmap="magma")
                axes[1, 2].set_title(
                    "High-Frequency Map", fontsize=11, fontweight="bold"
                )
                axes[1, 2].axis("off")

                plt.tight_layout()
                panel_path = os.path.join(
                    vis_dir, f"epoch_{epoch:02d}_sample_{bname}.png"
                )
        ...
        if os.path.exists("/content/drive/MyDrive") and epoch in [5, 10, 15, 20]:
            try:
                ...
                shutil.copy(
                    ema_best_model_path,
                    os.path.join(drive_path, "airnet_v1_2_ema_best_model.pth"),
                )
                shutil.copy(
                    history_csv_path, os.path.join(drive_path, "training_history.csv")
                )
                print(
                    f"  [OK] Backed up epoch {epoch} artifacts to Google Drive: '{drive_path}'"
                )
    ...
    with torch.no_grad(), torch.inference_mode():
        for lr_b, gt_b, fnames in val_loader:
            ...
            bic_b = torch.clamp(
                F.interpolate(
                    lr_b.float(), size=(256, 256), mode="bicubic", align_corners=False
                ),
                0.0,
                1.0,
            )
    ...
    with torch.no_grad(), torch.inference_mode():
        ...
        demo_bic_t = torch.clamp(
            F.interpolate(
                demo_lr_t.float(), size=(256, 256), mode="bicubic", align_corners=False
            ),
            0.0,
            1.0,
        )

    assert demo_lr_t.shape == (
        1,
        1,
        128,
        128,
    ), f"Expected input shape [1,1,128,128], got {demo_lr_t.shape}"
    assert demo_pred_t.shape == (
        1,
        1,
        256,
        256,
    ), f"Expected output shape [1,1,256,256], got {demo_pred_t.shape}"

    Image.fromarray((demo_lr_t.squeeze().cpu().numpy() * 255.0).astype(np.uint8)).save(
        os.path.join(final_demo_dir, "input_128.png")
    )
    Image.fromarray((demo_bic_t.squeeze().cpu().numpy() * 255.0).astype(np.uint8)).save(
        os.path.join(final_demo_dir, "bicubic_256.png")
    )
    Image.fromarray(
        (demo_pred_t.squeeze().cpu().numpy() * 255.0).astype(np.uint8)
    ).save(os.path.join(final_demo_dir, "airnet_v1_2_256.png"))
    Image.fromarray((demo_gt_t.squeeze().cpu().numpy() * 255.0).astype(np.uint8)).save(
        os.path.join(final_demo_dir, "ground_truth_256.png")
    )
    ...
    stage3_manifest = {
        ...
        "best_epoch": (
            int(np.argmax([r["val_psnr"] for r in v1_2_rows])) + 1
            if v1_2_rows
            else total_epochs
        ),
        "best_metrics": {
    ...
    index_rows = [
        {
            "stage": "Stage 3",
            "artifact": "EMA Best Checkpoint",
            "type": "PTH",
            "path": os.path.relpath(ema_best_model_path, stage3_root),
            "exists": True,
            "description": "EMA best checkpoint for AIR-Net v1.2",
        },
        {
            "stage": "Stage 3",
            ...
            "exists": True,
            "description": "Epoch-by-epoch training metrics",
        },
        {
            "stage": "Stage 3",
            "artifact": "v1 vs v1.2 Comparison CSV",
            "type": "CSV",
            "path": "comparison/v1_vs_v1_2_metrics.csv",
            "exists": True,
            "description": "320-sample comparison matrix",
        },
        {
            "stage": "Stage 3",
            "artifact": "v1 vs v1.2 Report",
            "type": "TXT",
            "path": "comparison/v1_vs_v1_2_report.txt",
            "exists": True,
            "description": "AIR-Net v1 vs v1.2 comparative report",
        },
        {
            "stage": "Stage 3",
            ...
            "path": "visualizations/",
            "exists": True,
            "description": "Multi-panel visual comparison images",
        },
        {
            "stage": "Stage 3",
            "artifact": "Final Demo Images",
            "type": "PNG",
            "path": "final_demo/",
            "exists": True,
            "description": "Final 128x128 -> 256x256 test demo outputs",
        },
        {
            "stage": "Stage 3",
            "artifact": "Master Report",
            "type": "TXT",
            "path": "STAGE3_MASTER_REPORT.txt",
            "exists": True,
            "description": "Stage 3 master development report",
        },
        {
            "stage": "Stage 3",
            "artifact": "Master Manifest",
            "type": "JSON",
            "path": "stage3_manifest.json",
            "exists": True,
            "description": "Stage 3 machine-readable manifest",
        },
    ]
