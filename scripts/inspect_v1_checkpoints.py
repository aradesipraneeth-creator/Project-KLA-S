...
def main():
    ...
    for idx, path in enumerate(pth_files, start=1):
        ...
        print(f"     Epoch:    {epoch_val} | Best PSNR: {best_psnr_val} | Best SSIM: {best_ssim_val}")
        print(f"     EMA:      {ema_status} | Opt: {opt_status} | Sched: {sched_status} | Scaler: {scaler_status}")
    ...
    if valid_candidates:
        ...
        valid_candidates.sort(key=lambda x: x["epoch"] if isinstance(x["epoch"], int) else -1, reverse=True)
