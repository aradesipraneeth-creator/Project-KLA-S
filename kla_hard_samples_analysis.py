...
files = sorted([f for f in os.listdir(GT_FOLDER) if f.endswith(".npy")])
...
for idx, f in enumerate(files):
    ...
    sample_data.append({"file": f, "psnr": psnr, "ssim": ssim_score, "corr": corr})
...
worst_psnr = sorted(sample_data, key=lambda x: x["psnr"])[:10]

worst_ssim = sorted(sample_data, key=lambda x: x["ssim"])[:10]
...
for item in worst_psnr:
    ...
    lr = np.load(os.path.join(LR_FOLDER, file))

    lr_up = cv2.resize(lr, (256, 256), interpolation=cv2.INTER_CUBIC)

    fig, ax = plt.subplots(1, 2, figsize=(8, 4))

    ax[0].imshow(lr_up, cmap="gray")
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, f"WORST_PSNR_{file}.png"))
...
for item in worst_ssim:
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, f"WORST_SSIM_{file}.png"))
...
for item in worst_psnr:
    print(item["file"], "PSNR=", round(item["psnr"], 3))
...
for item in worst_ssim:
    print(item["file"], "SSIM=", round(item["ssim"], 3))
