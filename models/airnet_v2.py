...
class AIRNetV2(nn.Module):
    """
    AIR-Net v2: Multi-Objective High-Fidelity Semiconductor Image Restoration Network.
    
    Key Architectural Advancement (Section 13 Residual Strategy):
        Restored Output = Bicubic(Input) + Residual(Input)
    
    This guarantees a baseline starting performance of PSNR >= 22.98 dB at Epoch 1
    and allows AIR-Net v2 to focus entirely on learning high-frequency semiconductor
    detail restoration and noise removal to target PSNR >= 25 dB.
    """
    def __init__(
        self,
        in_channels: int = 1,
    ...
    def forward(self, inp: torch.Tensor) -> Dict[str, Any]:
        ...
        dec3_feat = self.decoder_level3(dec3)
