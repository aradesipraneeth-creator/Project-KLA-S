...
def test_main():
    ...
    print(
        f"Predicted Noise Score: {noise_score.item():.4f} ({noise_score.item() * 100:.1f}%)"
    )
    print(
        f"Predicted Blur Score:  {blur_score.item():.4f} ({blur_score.item() * 100:.1f}%)"
    )
    print(
        f"Predicted Texture Score: {texture_score.item():.4f} ({texture_score.item() * 100:.1f}%)"
    )
    ...
    assert restored.shape == (
        ...
        256,
        256,
    ), f"Expected Restored shape (1, 1, 256, 256), got {restored.shape}"
    assert edge.shape == (
        1,
        1,
        256,
        256,
    ), f"Expected Edge shape (1, 1, 256, 256), got {edge.shape}"
    assert (
        0.0 <= noise_score.item() <= 1.0
    ), f"Noise score out of bounds: {noise_score.item()}"
    assert (
        0.0 <= blur_score.item() <= 1.0
    ), f"Blur score out of bounds: {blur_score.item()}"
    assert (
        0.0 <= texture_score.item() <= 1.0
    ), f"Texture score out of bounds: {texture_score.item()}"
    ...
    assert gt_edges.shape == (
        1,
        1,
        256,
        256,
    ), f"Expected GT edges shape (1, 1, 256, 256), got {gt_edges.shape}"
    print(
        f"[OK] Sobel Edge Target Generator PASSED. GT Edge map shape: {tuple(gt_edges.shape)}"
    )

    criterion = AIRNetHybridLoss(
        l1_weight=0.60, ssim_weight=0.25, edge_weight=0.15, use_lpips=False
    )
    loss_val = criterion(out_dict, dummy_gt)
    print(
        f"[OK] AIRNetHybridLoss Forward Pass PASSED. Calculated Loss: {loss_val.item():.6f}"
    )
